...
function updateCart() {
  ...
  cartData.forEach(i => {
    ...
    const itemName = currentLang === "en" ? i.name : i.ar;
    li.innerHTML = `<span>${itemName} x${i.qty} = ${i.price * i.qty} MAD</span>
...
function applyTranslations() {
  ...
  productList.querySelectorAll(".product h3").forEach((h3, idx)=>{
    ...
    if(product) h3.textContent = currentLang==="en"?product.name:product.ar;
  });
