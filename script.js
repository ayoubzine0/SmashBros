...
if (path.includes("becane")) {
  products = [
    ...
    { id: 2, name: "Becane Headlight", ar: "ضوء أمامي بيكان", price: 250, stock: 10, img: "https://i.imgur.com/J6l8Ln2.jpeg" },
  ];
...
function addToCart(product) {
  ...
  else cartData.push({ ...product, qty, page: window.location.pathname, name: product.name, ar: product.ar });
  saveCart();
  updateCart();
...
function updateCart() {
  ...
  let total = 0;
  cartData.forEach(i => {
    const displayName = currentLang === "en" ? i.name : i.ar;
    const li = document.createElement("li");
    li.innerHTML = `<span>${displayName} x${i.qty} = ${i.price * i.qty} MAD</span>
